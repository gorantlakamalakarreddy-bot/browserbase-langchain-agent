...
def test_search_empty_query_returns_error():
    ...
    result = browserbase_search.invoke({"query": "   "})
...
def test_fetch_rejects_non_http_url():
    ...
    result = browserbase_fetch.invoke({"url": "file:///etc/passwd"})
    assert "Invalid URL" in result or "not allowed" in result


def test_fetch_rejects_localhost():
    from browser_agent.tools import browserbase_fetch

    result = browserbase_fetch.invoke({"url": "http://localhost/admin"})
    assert "Invalid URL" in result or "not permitted" in result


def test_fetch_rejects_private_ip():
    from browser_agent.tools import browserbase_fetch

    result = browserbase_fetch.invoke({"url": "http://192.168.1.1/"})
    assert "Invalid URL" in result or "not permitted" in result
...
def test_rendered_extract_calls_stagehand_and_releases_session():
    ...
    with (
        patch("browser_agent.tools._bb") as mock_bb,
...
def test_rendered_extract_rejects_internal_url():
    from browser_agent.tools import browserbase_rendered_extract
